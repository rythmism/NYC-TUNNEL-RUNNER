...
*
*   Desktop Build: gcc main.c -o nyc_tunnel_runner -lraylib -lopengl32 -lgdi32 -lwinmm
*   Web Build: emcc main.c -o index.html -DPLATFORM_WEB -I"C:/Program Files (x86)/raylib/raylib/src" "C:/Program Files (x86)/raylib/raylib/src/libraylib.a" -s USE_GLFW=3 -s ASYNCIFY -O2
*
********************************************************************************************/
...
static void InitAudioAssets(void)
{
    ...
    float scale[8] = { 146.83f, 155.56f, 184.99f, 196.00f, 220.00f, 233.08f, 261.63f, 293.66f };
...
static void DrawCharacter(OutfitType outfit, float x, float y, float w, float h, bool ducking, bool blink)
{
    ...
    switch (outfit)
    {
        ...
        {
            DrawRectangleRounded(body, 0.3f, 6, (Color){ 40, 90, 200, 255 }); 
            DrawCircleV(headPos, headR, skin);
            DrawRectangle(headPos.x - headR, headPos.y - headR * 0.9f, headR * 2, headR * 0.6f, (Color){ 230, 120, 30, 255 }); 
        } break;

        case OUTFIT_MTA:
        {
            DrawRectangleRounded(body, 0.2f, 6, (Color){ 255, 165, 0, 255 }); 
            DrawRectangle(body.x, body.y + body.height * 0.25f, body.width, 6, (Color){ 255, 255, 120, 255 }); 
            DrawCircleV(headPos, headR, skin);
            DrawRectangle(headPos.x - headR * 1.05f, headPos.y - headR * 1.1f, headR * 2.1f, headR * 0.7f, (Color){ 245, 200, 30, 255 }); 
        } break;

        case OUTFIT_TOURIST:
        {
            DrawRectangleRounded(body, 0.3f, 6, (Color){ 230, 60, 90, 255 }); 
            DrawRectangle(body.x, body.y + body.height * 0.3f, body.width, body.height * 0.15f, (Color){ 250, 220, 60, 255 });
            DrawCircleV(headPos, headR, skin);
            DrawRectangle(headPos.x - headR, headPos.y - headR * 0.8f, headR * 2, headR * 0.5f, (Color){ 255, 255, 255, 255 }); 
            DrawRectangle(body.x + body.width - 4, body.y + body.height * 0.2f, 10, 8, (Color){ 20,20,20,255 }); 
        } break;

        case OUTFIT_HASIDIC:
        {
            DrawRectangleRounded(body, 0.15f, 6, (Color){ 25, 25, 30, 255 }); 
            DrawRectangle(body.x, body.y, body.width, body.height, (Color){ 25,25,30,255 });
            DrawCircleV(headPos, headR, skin);
            DrawRectangle(headPos.x - headR * 1.1f, headPos.y - headR * 0.6f, headR * 2.2f, headR * 0.35f, (Color){ 15,15,15,255 }); 
            DrawRectangle(headPos.x - headR * 0.6f, headPos.y - headR * 1.2f, headR * 1.2f, headR * 0.7f, (Color){ 15,15,15,255 }); 
        } break;
...
static void SpawnCollectible(void)
{
    for (int i = 0; i < MAX_COLLECTIBLES; i++)
    {
        ...
        {
            bool isBill = GetRandomValue(0, 99) < 18; 
...
/* ---------------------------------------------------------------------------------------
   EMSCRIPTEN FRAME ENGINE CALLBACK
--------------------------------------------------------------------------------------- */
